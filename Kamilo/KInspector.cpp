...
public:
	...
	static KTEXID makeIconTexture() {
		...
		K_assert(strlen(dat) == ICON_BMP_W * ICON_BMP_H);
		K_assert(ICON_BMP_W <= ICON_TEX_W);
		K_assert(ICON_BMP_H <= ICON_TEX_H);
...
public:
	...
	void updatePanel() {
		...
		K_assert(m_visible_list.size() > 0);
...
bool KDebugGui::K_DebugGui_InputBlend(const char *label, KBlend *blend) {
	...
	K_assert(blend);
	K_assert(s_table[KBlend_ENUM_MAX] == nullptr);
	int val = *blend;
...
bool KDebugGui::K_DebugGui_InputFilter(const char *label, KFilter *filter) {
	...
	K_assert(filter);
	K_assert(s_table[KFilter_ENUM_MAX] == nullptr);
	int val = *filter;
