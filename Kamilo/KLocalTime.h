...
struct KLocalTime {
	...
	static constexpr char *DEFAULT_TIME_FORMAT = "%Y-%m-%d %H:%M:%S";
	...
	explicit KLocalTime(const char *s, const char *fmt=DEFAULT_TIME_FORMAT);
	...
	/// fmt に nullptr を指定した場合はデフォルトのフォーマット DEFAULT_TIME_FORMAT を使う
	...
	///      この値に nullptr を指定した場合は、デフォルトのフォーマット DEFAULT_TIME_FORMAT に従っているものとみなす
